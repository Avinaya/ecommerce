...
function CartCheckOut(props) {
  ...
  const [{ onlineCart }] = useStateValue();
  const [{ basket }, dispatch1] = useStateValue();
  const totalQuantity = basket.reduce(
    (totalItem, basket) => totalItem + basket.productQuantity,
    0
  );
  ...
  useEffect(() => {
    if (user != null) {
      ...
      if (props.cart != null) {
        ...
        setUserTotalQuantity(
          data1.reduce((totalItem1, data1) => totalItem1 + data1.quantity, 0)
        );
        setUserTotalSalePrice(
          data1.reduce((totalSale1, data1) => totalSale1 + data1.price, 0)
        );
      }
      // if(onlineCart!=null){
      //   console.log("online cart",onlineCart);
      //   setCart(onlineCart);
      //   console.log("caart",onlineCart);

      // const data1=onlineCart;
      // setUserTotalQuantity(data1.reduce((totalItem1,data1) => totalItem1 + data1.quantity, 0));
      // setUserTotalSalePrice(data1.reduce((totalSale1,data1) => totalSale1 + (data1.price) , 0));
      // }
    } else {
  ...
  return (
    <div className="cart-tool-parent">
      ...
      <div className="cart-tool-parent-item cart-tool-parent-item-budget">
        <div className="cart-tool-parent-item-budget-summary">
          Order Summary
          {user != null ? (
            <span className="float-right">{userTotalQuantity}</span>
          ) : (
            <span className="float-right"> {totalQuantity}</span>
          )}{" "}
          item
        </div>
        <div className="cart-tool-parent-item-budget-subtotal">
          Subtotal
          {user != null ? (
            <span className="float-right">{userTotalSalePrice}</span>
          ) : (
        ...
        <div className="cart-tool-parent-item-budget-total">
          Total
          {user != null ? (
            <span className="float-right">{userTotalSalePrice}</span>
          ) : (
